...
# Danger!!! Awful formatting ahead! This module is a real disaster area, but it works
# and I don't wanna take the time to make it all pretty, since it already works and it's
# mostly readable. Yes I know, there's an even mix of f"" strings, "".format() strings,
# and "" + var + "" strings and there's no reason it should be that way, but this is what
# happens when you're fresh out of university and you write this trash at 2am while
# slammin coffee and donuts. Let this code serve as a memorial to my fuckery, and a
# warning to those who work in the early morning hours. Cheers!
...
class Game:
    ...
    async def createPlayers(self, rawPlayers):
        for player in rawPlayers:
            self.cursor.execute(
                "select wallet from money where id = ?", (player.id,))  # trailing comma creates tuple
            wallet = self.cursor.fetchone()
            if wallet:
                withdraw = wallet[0]
                # just in case negative money comes up somehow
                if withdraw <= 0:
                    withdraw = 100
                    newPlayer = Player(player.name, player.id, withdraw)
                    await self.ctx.send(
                        f"Welcome back, {player.name}! You ran out of money, "
                        + "but you sold us your car for 100 feelbucks!"
                    )
                else:
                    newPlayer = Player(player.name, player.id, withdraw)
                    await self.ctx.send(
                        f"Welcome back, {player.name}! You currently have {withdraw} feelbucks!"
                    )
            else:
                newPlayer = Player(player.name, player.id, 100)
                self.cursor.execute(
                    "insert into money values (?, ?)", (player.id, 100))
                await self.ctx.send(
                    f"Looks like you're new here, {player.name}. You'll start with 100 feelbucks!"
                )
    ...
    async def gatherPlayers(self, isFirstGame):
        ...
        gatherMessage = await self.ctx.send(
            'A new blackjack game is starting in 15 seconds! Type "join" to enter! Max number of players is '
            + str(PLAYER_LIMIT)
            + "."
        )
        await sleep(15)
        ...
        for message in messages:
            ...
            if (message.content.lower() == "join"
                    and message.author not in rawPlayers
                    and message.author.id not in self.allPlayerSet):
    ...
    def cashOut(self, player):
        self.cursor.execute(
            "update money set wallet = ? where id = ?", (player.getWallet(), player.getID()))
    ...
    async def removePlayers(self):
        ...
        removeMessage = await self.ctx.send(
            'This hand is over, but we\'ll start again in 15 seconds. Type "stay" to remain at the table.'
        )
        await sleep(15)
        ...
        for player in list(iter(self.currentPlayers)):
            if player.getWallet() == 0:
                await self.removeIdlePlayer(
                    player,
                    set([player.getID()]),
                    f"{player.getName()}, you ran out of feelbucks, I know that feel... "
                    + "If you join again you'll get 100 feelbucks... for a price.",
                )
            else:
                await self.removeIdlePlayer(
                    player,
                    removeSet,
                    f"{player.getName()}, you'll be leaving with {player.getWallet()} feelbucks. See ya next time!"
                )
    ...
    async def gatherBets(self):
        removeSet = set(iter(self.currentPlayerIDs))
        betMessage = await self.ctx.send(
            'Time to place your bets! Values greater than your wallet will bet your entire remaining balance. '
            + 'Betting ends in 15 seconds!\n**Type "bet <value>" to place your bet**' +
            self.allWalletsStr()
        )
        await sleep(15)
        ...
        for player in list(iter(self.currentPlayers)):
            await self.removeIdlePlayer(
                player,
                removeSet, f"Sorry {player.getName()}, you were removed for inactivity..."
            )
    ...
    async def dealCards(self):
        ...
        await self.ctx.send(
            "The cards have been dealt, let's see what we got..."
            + self.allHandsStr()
            + "Take your time to inspect the table! The game continues in 15 seconds..."
        )
        await sleep(15)
    ...
    async def checkNaturals(self):
        ...
        if self.dealer.getHandValue() == 21:
            await self.ctx.send(
                "Woblakjak! Any players who failed to get a natural will lose their bet...\n"
                + self.dealer.handShowStr()
            )
            await sleep(3)
            for player in self.currentPlayers:
    ...
    async def playHands(self, player):
        await self.ctx.send(
            "It's your turn, "
            + player.getName()
            + "! You have 10 seconds for each choice.\n"
            + self.dealer.handHideStr()
            + " Wojak's hand\n--"
        )
        await sleep(2)
        while player.stillPlaying():
            idle = True
            optionMessage = await self.ctx.send(f"{player.currentHandStr()}\nOptions: {player.optionsStr()}")
            await sleep(10)
            ...
            for message in messages:
                if message.author.id == player.getID():
                    if message.content.lower() == "hit":
                        ...
                        player.split()
                        await self.ctx.send(
                            "Split! Your two hands are now:\n"
                            + player.allHandsStr()
                            + "\nYou will play each hand regularly, in succession..."
                        )
                        await sleep(3)
                        idle = False
                        break
            ...
            if idle:
                await self.removeIdlePlayer(
                    player,
                    set([player.getID()]),
                    "Sorry {0}, you were removed for inactivity and current bets have been lost...".format(
                        player.getName())
                )
                break
    ...
    async def serveDealer(self):
        ...
        while self.dealer.stillPlaying():
            ...
            if self.dealer.getHandValue() > 21:
                await self.ctx.send(
                    "I bust! Players who didn't bust will win their bet!\n{0}".format(
                        self.dealer.handShowStr())
                )
    ...
    async def playBlackjack(self):
        await self.ctx.send(f"Alright, the players are rounded up. The order of play will be:\n{self.allPlayersStr()}")
        await sleep(2)
        await self.ctx.send(
